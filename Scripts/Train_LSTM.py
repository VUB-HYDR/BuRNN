...
if __name__ == "__main__":
    ...
    def objective(trial, log_path, zero=0):
        ...
        seed = SEED
        #loss_f, num_targets, seperate_heads = partial(Binary_and_MSELoss, bce_loss_fn=bce_loss_fn, lambda_factor=lambda_factor, alpha=0.5, zero=zero), len(targets)*2, True

        #loss_f, dynamic_loss, num_targets = partial(MSELoss, lambda_factor=lambda_factor), False, len(targets)
        loss_f, num_targets, seperate_heads = partial(Binary_and_NLLLoss, bce_loss_fn=bce_loss_fn, lambda_factor=lambda_factor, alpha=0.5, zero=zero), len(targets)*3, True
        #loss_f, num_targets, seperate_heads = Gaussian_NLLLoss, len(targets)*2, True
        model = LightningLSTM(input_size=len(features+static_features), hidden_size=hidden_size, num_layers=num_layers, lstm_activ_f=activ_f1, linear_activ_f=activ_f2, output_size=num_targets, dropout=dropout, loss_f=loss_f, linear=linear, dynamic_loss=dynamic_loss, lr=learning_rate, spinup_length=spinup_length, ba_as_predictor=ba_as_predictor, seed=seed, seperate_heads=seperate_heads)
    ...
    for test_idx, test_fold in enumerate(folds[::-1]):
        ...
        for val_idx, val_folds in enumerate(train_val_folds):
            dm.change_val_test_folds(val_folds = val_folds, test_folds = [test_fold], normalize_targets=not bool(val_idx))
            zero = dm.get_target_zero()
            ...
            objective_f = partial(objective, log_path = val_path, zero=zero)
